...
def test_tab_and_shift_tab_reach_key_press_event_instead_of_stealing_focus(qapp):
    ...
    # never reaches translate_key_event and the shell's tab-completion is
...
def test_rapid_resize_churn_does_not_wipe_already_rendered_content(qapp, tmp_path, monkeypatch):
    ...
    # user ever sees it. Confirmed independently: feeding "~/project $ "
    ...
    widget._on_output("~/project $ ")
    assert "~/project $" in widget.screen._screen.display[0]
...
def test_enter_with_content_appends_to_history(qapp):
    widget = TerminalWidget()
    widget.screen = TerminalScreen(columns=40, lines=15)
    widget.screen.feed("~/project $ git status")

    QTest.keyClick(widget, Qt.Key_Return)

    assert widget.get_history() == ["~/project $ git status"]
